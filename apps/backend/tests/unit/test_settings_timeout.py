...
need more than 600s) but bounded so a stuck request can't hold a worker
...
class TestRequestTimeoutSetting:
    def test_default_is_600(self):
        assert Settings.model_fields["request_timeout_seconds"].default == 600
    ...
    def test_blank_string_falls_back_to_default(self):
        # A blank env var (REQUEST_TIMEOUT_SECONDS=) must not crash; defaults to 600.
        assert Settings(request_timeout_seconds="").request_timeout_seconds == 600

    def test_garbage_falls_back_to_default(self):
        assert Settings(request_timeout_seconds="abc").request_timeout_seconds == 600
    ...
    def test_infinity_falls_back_to_default(self):
        ...
        assert Settings(request_timeout_seconds="inf").request_timeout_seconds == 600

    def test_nan_falls_back_to_default(self):
        assert Settings(request_timeout_seconds="nan").request_timeout_seconds == 600
